...
std::string convertTimeFormat(const std::string& input) {
    ...
    return input.substr(6,4)+"-"+input.substr(3,2)+"-"+
           input.substr(0,2)+" "+input.substr(11,8);
}
...
bool DatabaseManager::testConnection() {
    try {
        ...
        if (c.is_open()) { std::cout << "PostgreSQL OK\n"; return true; }
...
bool DatabaseManager::insertLocationData(const LocationData& d) {
    try {
        ...
        } else if (d.networkType == "GSM") {
            res = txn.exec_params(
                "INSERT INTO gsm_data(latitude,longitude,altitude,accuracy,time,networkType,"
                "cell_id,gsm_bsic,gsm_arfcn,gsm_lac,gsm_mcc,gsm_mnc,gsm_psc,gsm_dbm,gsm_timing_advance)"
...
void fetchDBPoints() {
    try {
        ...
        pqxx::result res = txn.exec(
            ...
            "  SELECT latitude, longitude, time FROM lte_data "
            "  UNION ALL SELECT latitude, longitude, time FROM nr_data "
            "  UNION ALL SELECT latitude, longitude, time FROM gsm_data "
            ") t WHERE latitude!=0 AND longitude!=0 "
            "ORDER BY time DESC LIMIT " + std::to_string(g_maxDbPoints));
