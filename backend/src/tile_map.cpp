...
double latToMercY(double lat) {
    ...
    return std::log(std::tan(M_PI / 4.0 + rad / 2.0)) * 180.0 / M_PI;
}

std::string getTilePath(int zoom, int tx, int ty) {
    return "tiles/" + std::to_string(zoom) + "/" +
...
size_t onPullResponse(void* data, size_t size, size_t nmemb, void* userp) {
    ...
    blob.insert(blob.cend(),
        static_cast<unsigned char*>(data),
        static_cast<unsigned char*>(data) + realsize);
    return realsize;
}

void FetchWorker() {
    CURL* curl = curl_easy_init();
    if (!curl) { std::cerr << "[TileLoader] curl init failed\n"; return; }
    ...
    while (true) {
        ...
        unsigned char* pixels = stbi_load_from_memory(
            rawBlob.data(), (int)rawBlob.size(), &w, &h, &ch, STBI_rgb_alpha);
        if (!pixels) {
            std::lock_guard<std::mutex> lk(g_CacheMutex);
            g_TileCache[job.id].isLoading = false;
            continue;
